  ...
  const subContent = (item: any) => (
    <div className='overflow-hidden transition-all duration-300 ease-in-out'>
      {item.children.map((child: any) => (
        <div
          key={child.key}
          className={`${collapsed ? 'pl-14' : ''} ${currentMenu === child.path ? 'bg-#e5f4ff' : ''} text-xs flex min-w-0 items-center hover:bg-gray/15 px-4 gap-4 h-32px cursor-pointer overflow-hidden whitespace-nowrap transition-all duration-200`}
    ...
    `}>
      ...
      <div className='select-none'>
        {menuList?.map((item: any) => (
          <div key={item.key} className='text-xs rounded-2px'>
            <div
